"""Bounded tensor signatures used by the external weight-sync patches."""
...
logger = logging.getLogger("weight_sync_debug.recorder")
...
def tensor_signature(tensor: torch.Tensor) -> dict[str, Any]:
    """Return a bounded signature; use ``HASH=full`` for a complete hash."""
...
def _emit_record(record: Mapping[str, Any]) -> None:
    ...
    if path is None or path in _FILE_ERROR_PATHS:
        return
    try:
...
class _DebugSession:
    def __init__(self, stage: str, context: Mapping[str, Any] | None = None, target: bool = False):
        ...
        pattern_name = "VERL_WEIGHT_SYNC_DEBUG_TARGET_NAMES" if target else "VERL_WEIGHT_SYNC_DEBUG_NAMES"
        self.pattern = _compile_pattern(pattern_name)
    ...
    def log(self, name: str, tensor: torch.Tensor, **extra: Any) -> None:
        if not enabled() or not self.active or (self.max_tensors and self.logged >= self.max_tensors):
            return
        if self.pattern is not None and self.pattern.search(name) is None:
            return
        self.logged += 1
        _emit_record(
            {

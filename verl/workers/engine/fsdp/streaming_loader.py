...
def _add_kimi_packed_tasks(
    ...
    The destination is the packed Kimi ABI: gate_up is
    [expert, hidden, 2 * intermediate], down is [expert, intermediate, hidden].
    """
...
def _check_shape(task: _LoadTask, source_shape: tuple[int, ...]) -> None:
    if task.allow_prefix_crop:
        expected = task.expected_source_shape
        if (
            len(source_shape) != len(expected)
            or source_shape[0] < expected[0]
            or source_shape[1:] != expected[1:]
        ):
            raise RuntimeError(
    ...
    *,
    ...
    for target_name, spec in parameters.items():
        ...
        if target_name.endswith((".experts.gate_up_proj", ".experts.down_proj")) and len(spec.global_shape) == 3:
            _add_kimi_packed_tasks(spec, weight_map, tasks_by_file, strict)
            continue
    ...
    if missing_parameters:
        logger.warning(
            "Streaming Kimi checkpoint skipped %d missing parameters: %s",

...
def _materialize_source_expert_shard(name: str, param: DTensor, device):
    ...
    return local_param, ep_dim, source_ep, source_start, source_local
...
def export_kimi_source_local_param(name: str, param: DTensor, device):
    ...
    local_param, _, _, source_start, _ = _materialize_source_expert_shard(
        name, param, device
    )
    return f"{name}{KIMI_PACKED_LOCAL_MARKER}{source_start}", local_param
...
def _source_ep_rank_by_global_rank(mesh, ep_dim: int) -> dict[int, int]:
    _, rank_map = _mesh_rank_map(mesh)
...
def export_kimi_packed_local_param(
    ...
    local_param, ep_dim, source_ep, source_start, source_local = (
        _materialize_source_expert_shard(name, param, device)
    )
    num_experts = int(param.shape[0])
